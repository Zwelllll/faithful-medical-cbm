"""GPU-only entry point for Stage 12 strict joint CBM training on one development fold."""
...
from ..models.joint_cbm import JointCBM, CONCEPT_ORDER
from ..reproducibility import seed_everything
from .baseline import development_fold_loaders
from .joint_cbm import fit_fold, validate_config, check_order, training_pos_weights, prevalence
...
def run_training(config_path: Path, fold: int, run_name: str) -> dict:
    ...
    model_type = config["joint_training"]["model_type"]
    namespace = "joint_soft" if model_type == "joint_soft" else "joint_hard"
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Full joint training requires CUDA; use offline unit smoke tests on CPU")
    seed_everything(**config["reproducibility"])
    factory = LoaderFactory(config_path)
    loaders = development_fold_loaders(factory, fold)
    missing = [str(path) for loader in loaders.values() for path in loader.dataset.paths if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Missing development dermoscopic files: {missing[:5]} ({len(missing)} total)")
    ...
    artifact_dir = config["paths"]["artifacts"] / namespace / run_name / f"fold_{fold}"
    checkpoint_dir = config["paths"]["checkpoints"] / namespace / run_name / f"fold_{fold}"
    ...
    try:
        ...
        commit = None
    provenance = {
        "model_type": model_type, "locked_test_used": False,
        "loss_weights": {"concept": 1.0, "diagnosis": 1.0},
        "model_label": "JOINT HARD CBM WITH STRAIGHT-THROUGH ESTIMATOR" if model_type == "joint_hard_ste" else "JOINT SOFT CBM",
    ...
    model = JointCBM(model_type, pretrained=True).to(torch.device("cuda"))
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/joint_soft.toml"))
