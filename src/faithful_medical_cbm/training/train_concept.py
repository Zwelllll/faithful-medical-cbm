"""GPU-only entry point for Stage 5 one-concept Fold 0 sanity check."""
...
from ..models.concept import ConceptEfficientNet
from ..reproducibility import seed_everything
from .baseline import development_fold_loaders
from .concept import fit_fold, validate_settings, checked_target_index, training_pos_weight, prevalence
...
def run_training(config_path: Path, fold: int, run_name: str) -> dict:
    ...
    if type(fold) is not int or fold != 0:
        raise ValueError("Stage 5 permits Fold 0 only")
    ...
    index = checked_target_index(factory.concept_columns, config)
    weight = training_pos_weight(loaders["train"].dataset, index)
    counts = {role: prevalence(loader.dataset, index) for role, loader in loaders.items()}
    ...
    artifact_dir = config["paths"]["artifacts"] / "concept_sanity" / run_name / f"fold_{fold}"
    checkpoint_dir = config["paths"]["checkpoints"] / "concept_sanity" / run_name / f"fold_{fold}"
    ...
    try:
        ...
        commit = None
    provenance = {
        "concept": config["concept_training"]["target"], "concept_order": list(factory.concept_columns),
        "concept_prevalence": counts, "pos_weight": weight, "pos_weight_source": "Fold 0 training labels only",
    ...
    model = ConceptEfficientNet(pretrained=True).to(torch.device("cuda"))
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/concept_sanity.toml"))
    parser.add_argument("--fold", type=int, required=True, choices=[0])
